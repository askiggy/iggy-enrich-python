...
class LocalIggyDataPackage(IggyDataPackage):
    ...
    def set_geoms_loc(cls, v, values):
        return v or str(
            ...
            / f"{values['crosswalk_prefix']}_{values['iggy_version_id']}"
        )
    ...
    def enrich(
        ...
        if type(points_) == gpd.GeoDataFrame:
            points_["qk"] = points_.geometry.apply(
                lambda p: quadkey.from_geo((p.y, p.x), level=zoom)
            )
        else:
            points_["qk"] = points_.apply(
                lambda row: str(
                    quadkey.from_geo(
                        (row[latitude_col], row[longitude_col]), level=zoom
                    )
                ),
                axis=1,
            )
